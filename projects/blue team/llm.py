...
def _ensure_credentials(model_choice: str, llm_class, model_params: dict) -> None:
    ...
    if "ChatAnthropic" in class_name:
        _require(conf.ANTHROPIC_API_KEY, "ANTHROPIC_API_KEY", "Anthropic")
    elif "ChatGoogleGenerativeAI" in class_name:
        _require(conf.GOOGLE_API_KEY, "GOOGLE_API_KEY", "Google Gemini")
    elif "ChatOpenAI" in class_name:
        base_url = (model_params or {}).get("base_url", "").lower()
        if "openrouter" in base_url:
            _require(conf.OPENROUTER_API_KEY, "OPENROUTER_API_KEY", "OpenRouter")
        elif "groq" in base_url:
            _require(conf.GROQ_API_KEY, "GROQ_API_KEY", "Groq")
        else:
            _require(conf.OPENAI_API_KEY, "OPENAI_API_KEY", "OpenAI")

def refine_query(llm, user_input):
    system_prompt = """You are a Cybercrime Threat Intelligence Expert. Your task is to refine the provided user query that needs to be sent to darkweb search engines. 
    ...
    try:
        refined = chain.invoke({"user_input": user_input})
        return refined.strip()
    except Exception as e:
        logging.warning("Failed to refine query: %s", e)
        return user_input

def filter_results(llm, query, results):
    results_str = "\n".join([f"{i}. {r['title']} - {r['link']}" for i, r in enumerate(results)])
    system_prompt = """You are a Cybercrime Threat Intelligence Expert. You are given a dark web search query and a list of search results in the form of index, link and title. 
...
PRESET_PROMPTS = {
    ...
    "malware": """You are a Malware and Ransomware Intelligence Expert tasked with analyzing dark web data for malware-related threats.
    ...
    "personal": """You are a Personal Threat Intelligence Expert tasked with analyzing dark web data for identity and personal information exposure.
    ...
    "corporate": """You are a Corporate Intelligence Expert tasked with analyzing dark web data for corporate data leaks and espionage activity.
...
def generate_summary(llm, query, content, preset="threat_intel", custom_instructions=""):
    ...
    try:
        ...
        logging.warning("Failed to generate summary: %s", e)
